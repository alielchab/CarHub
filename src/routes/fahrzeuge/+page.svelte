<script>
    ...
    let activeCars = $derived(
        data.cars.filter((car) => car.status === "aktiv"),
    );
    ...
    function carsForFilterOptions(ignore) {
        return activeCars.filter((car) => carMatchesFilters(car, ignore));
    }

    let filteredCars = $derived(
        activeCars.filter((car) => carMatchesFilters(car)),
    );
    ...
    let modelle = $derived(
        selectedMarke
            ? countValues(carsForFilterOptions("modell"), "modell")
            : [],
    );
    ...
    let sortedCars = $derived(
        [...filteredCars].sort((a, b) => {
            switch (selectedSort) {
                ...
                case "jahrgang_asc":
                    return (
                        toDateTime(a.inverkehrsetzung) -
                        toDateTime(b.inverkehrsetzung)
                    );
...
<section class="public-filter-section">
    <div class="section-heading">
        <h1><span>UNSERE</span> FAHRZEUGE</h1>
    ...
    <div class="public-filter-grid">
        ...
        <select bind:value={selectedZustand}>
            <option selected disabled value="">Zustand</option>
        ...
        <select bind:value={selectedGetriebe}>
            <option selected disabled value="">Getriebe</option>
    ...
    <div class="filter-bottom">
        ...
        <div class="result-message">
            {#if sortedCars.length === 0}
                Momentan wurden noch keine Fahrzeuge inseriert.
